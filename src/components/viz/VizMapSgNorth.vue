<template>
<div>
  <div class="wrapper__map-sg-north">
    <svg id="map-sg-north" />
  </div>
</div>
</template>
...
export default {
  ...
  methods: {
    async parseData() {
      ...
      this.$nextTick(() => {
        this.initChart();
        this.drawMapBySelection('North')        
      });           
    },
    initChart() {
      const chartSVG = d3.select('#map-sg-north');
      // draw base
      chartSVG
    ...
    drawMapBySelection(selectedCluster) {
      ...
      var rc = rough.svg(document.getElementById('map-sg-north'));

      const svg = d3.select('#map-sg-north');
      svg
        .append('g')
        .attr('class', 'map')
        .lower();  
